...
template <typename Key, typename T, typename Compare = std::__equal_to<Key>,
...
void HashIterator<Key, T, Compare, Hash>::increment() {
  //find the smallest element bigger than *mIt, no dup keys
  pair<Key, T> currentKey = *mIt;
  pair<Key, T> min = currentKey;
  for (const list<pair<Key, T> >& bucket: *mHashmap->mElems){
    for (const pair<Key, T>& kvpair : bucket){
      if (kvpair.first > currentKey.first && kvpair.first < min.first){
        min = kvpair;
      }
    }
  }
